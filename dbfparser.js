...
var fsi = require('fs-iconv');
var iconv = require('iconv-lite');
//const iconv = fsi.iconv;
//const fs = fsi.fs;
var fs = require('fs');
const { count } = require('console');
let file = './Расписание 2 неделя.DBF';
...
fs.readFile(file, null, function read(err, data) {
    ...
    var message = iconv.decode(data, "cp866").toString().split('  ');
    let i = 0;
    let count = 0;
    let flag = true;
    let mainArr = []
    ...
    message.forEach(element => {
        ...
        if (element.length !== 0 && flag) {
            ...
            if(i == 5 && element.replace(/ /g, '').length < 3) {
                ...
                if(count == 0){
                    obj[Object.keys(model)[i]] = element.replace(/[A-Z]/g, '').toString().split(' ')[2]
        ...
        if (i > 8){
            //console.log('-------------------------');
            i = 0
            if(Object.keys(obj).length!==0) mainArr.push(obj)
            obj = {}
        }
    ...
    console.log(mainArr);
    //console.log(message);
    fs.appendFile('test.json', JSON.stringify(result), function (err) {
        if (err) throw err;
        console.log('Saved!');
    });
});
