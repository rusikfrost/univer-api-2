...
/**
 * Checks if a rating already exists excluding itself
 * @param {string} id - id of item
 * @param {string} name - name of item
 */
const ratingExistsExcludingItself = async (id, name) => {
  return new Promise((resolve, reject) => {
    model.findOne(
      ...
      (err, item) => {
        utils.itemAlreadyExists(err, item, reject, 'RATINGS_ALREADY_EXISTS')
        resolve(false)
      }
    )
  })
...
/**
 * Checks if a rating already exists in database
 * @param {string} name - name of item
 */
const ratingExists = async (name) => {
  return new Promise((resolve, reject) => {
    model.findOne(
      {
        name
...
exports.updateItem = async (req, res) => {
  try {
    req = matchedData(req)
    const id = await utils.isIDGood(req.id)
    const doesRatingExists = await ratingExistsExcludingItself(id, req.name)
    if (!doesRatingExists) {
...
exports.createItem = async (req, res) => {
  try {
    req = matchedData(req)
    const doesRatingExists = await ratingExists(req.name)
    if (!doesRatingExists) {
